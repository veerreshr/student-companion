<?php  
            ...
            function row($time,$day,$today){
                ...
                $query="select * from week where day='$day' and id='".$_SESSION['id']."' and lastupdate between '$from' and '$to'";
                $result = mysqli_query($db, $query) or die(mysqli_error($db)); 
?> 
 
            <div id="<?php echo $day; ?>">
            <div class="firstcol"><?php echo $day ?></div>
                <?php
            ...
            ?>
            <div class="subject <?php if($time==$today)echo 'today ';
            if($rowcount!=0){
                $row= mysqli_fetch_assoc($result);
                if($row['holiday']==1){
                    echo "holiday";
                }elseif($row['present']==1){
                    echo "present";
